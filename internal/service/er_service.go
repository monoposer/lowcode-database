...
func (s *LowcodeService) GetERDiagram(ctx context.Context, _ *apiv1.GetERDiagramRequest) (*apiv1.GetERDiagramResponse, error) {
	...
	tables, err := meta.Query(ctx, `
		SELECT name, schema_name, table_name FROM lc_tables WHERE tenant_id = $1 ORDER BY name`, tid)
	if err != nil {
		return nil, err
	}
	defer tables.Close()
	...
	for tables.Next() {
		var name, schemaName, physName string
		if err := tables.Scan(&name, &schemaName, &physName); err != nil {
			return nil, err
		}
	...
	relRows, err := meta.Query(ctx, `
		SELECT id, name, kind, source_table_id, source_column_id, target_table_id, target_column_id
	...
	for relRows.Next() {
		...
		if err := relRows.Scan(&e.Id, &e.Label, &e.Kind, &e.SourceTableId, &srcCol, &e.TargetTableId, &tgtCol); err != nil {
			return nil, err
		}
		if srcCol != nil {
	...
	for colRows.Next() {
		...
		if typeID == "relationship" && cfgString(cfg, "link_column_id") != "" {
		...
		diagram.Edges = append(diagram.Edges, &apiv1.EREdge{
			...
			TargetColumnId: cfgString(cfg, "target_column_id"),
			Label:          colName,
		})
	}
